...
let _ =
  ...
  let pts = List.map (fun i ->
                let t = i *. (acos (-.1.)) /. 100. in
                let s = sin t in
                let c = cos t in
                let d = c ** 2. +. e2 *. (s ** 2.) in
                (e2 *. s *. c /. d, e /. d)) (range 100.) in
  ...
  let dists = List.map (fun p -> (H.dist p (0., 1.), H.dist p (-1., 0.5))) pts in
  P.scatterplot fp {P.default_graph with xmax = 10.; ymax = 10.} (List.map (fun p -> ((0., 0., 1.), [p])) dists);
