...
(*       let small_ee = exp (-.1.0) in *)
      ...
      let rec build' v1s v2s mns found =
        ...
        | (m, n) :: mns' ->
            if m = 0 && n = 0 then
              ...
              let quadded = apply_quad q2 v in
              let (vx, vy) = v in
              if verbose then Printf.printf "Lattice vector: (%.1f, %.1f). q1(v) = %f.\n" vx vy quadded;
              let found' = found || (quadded < small_vector_threshold) in
              if verbose then Printf.printf "(%.1f, %.1f, %.1f, %.1f) applied to (%.1f, %.1f) is %f." qa qb qb' qc (fst v) (snd v) quadded;
              (* check that above inequalities are satisfied *)
              let v1s' =
                if quadded >= small_rad1 && quadded <= rad1 then (
                  if verbose then Printf.printf "\t v1: yes";
                  v::v1s
                ) else v1s
              in
              let v2s' =
                if quadded >= small_rad2 && quadded <= rad2 then (
                  if verbose then Printf.printf "\t v2: yes";
                  v::v2s
                ) else v2s
              in
    ...
    let g_list =
      let rec build' gs v1v2s =
        ...
        | (v1, v2) :: v1v2s' ->
            ...
            if verbose then Printf.printf "v1: (%.1f, %.1f), v2: (%.1f, %.1f), \t g: (%.1f %.1f %.1f %.1f) " v11 v12 v21 v22 ga gb gc gd;
            (* condition for g to be in SL2(ZZ) *)
            let gs' =
              if det g = 1.0 then (
                if verbose then Printf.printf "Accept.";
                g::gs
              ) else gs
            in
            if verbose then Printf.printf "\n";
    ...
    | _ ->
        ...
        let dist_after_gamma g : gamma * float =
          ...
          if verbose then Printf.printf "(%.2f, %.2f) -- (%.2f, %.2f). g: (%.1f %.1f %.1f %.1f); d((%.1f, %.1f), (%.1f, %.1f)) = %f.\n"
